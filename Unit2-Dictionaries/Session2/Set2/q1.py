...
# rating scores. Your goal is to visit only the best-rated destinations. Write a function that takes 
# in a dictionary destinations and a rating_threshold as parameters. The function should iterate 
# through the dictionary and remove all destinations that have a rating strictly below the rating_threshold. Return the updated dictionary.
...
def remove_low_rated_destinations(destinations, rating_threshold):
    ...
    for destination, rating in destinations.items():
        if rating < rating_threshold:
            low_rating_set.add(destination)
            
    if not low_rating_set:
        return {}
    
    else:
        for destination in low_rating_set:
